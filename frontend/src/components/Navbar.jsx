...
export default function Navbar() {
    ...
    return (
        <>
            <div className={`max-w-screen-2xl container mx-auto md:px-20 px-4 dark:bg-slate-900 dark:text-white fixed top-0 right-0 left-0 z-50 ${sticy ? "sticky-navbar shadow-md bg-base-200 dark:bg-slate-600 dark:text-white duration-300 transition-all ease-in-out" : ""}`}>
                    ...
                    <div className="nevbar-end space-x-3">
                        ...
                        <div className='hidden md:block'>
                            <label className="input px-3 py-2 border rounded-md flex items-center gap-2">
                                <input type="text" className="bg-black grow outline-none dark:bg-slate-900 dark:text-white" placeholder="Search" />
                        ...
                        </label>
                        {
                            authUser ? (<Logout />) : (<div className="">
                                <a href='/Loginnew' className="bg-black text-white px-3 py-2 rounded-md hover:bg-slate-800 duration-300 cursor-pointer"
                                // onClick={() => document.getElementById("my_modal_3").showModal()}
                                >
                                    Login
                                </a>
                                {/* <button><Link to='/Loginnew' className='underline text-blue-500 cursor-pointer'>Login</Link></button> */}
                                {/* <Login /> */}
                            </div>
                            )}
